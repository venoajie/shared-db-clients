...
async def test_xadd_bulk_success(client):
    # FIX: redis-py pipeline() returns a Pipeline object.
    # Calling pipeline() is synchronous, but executing the pipeline is async.
    # The commands on the pipeline (xadd) are synchronous (they just queue).

    mock_pipe = MagicMock()  # xadd is sync
    mock_pipe.execute = AsyncMock()  # execute is async

    mock_pool = MagicMock()
    mock_pool.pipeline.return_value = mock_pipe

    # We must patch get_pool to return our mock_pool
